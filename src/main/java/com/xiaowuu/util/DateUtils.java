...
public class DateUtils {
	...
	public static SimpleDateFormat dateMinuteFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm");
	public static SimpleDateFormat dateChinaFormat = new SimpleDateFormat("yyyy年MM月");
	public static SimpleDateFormat longDateformat = new SimpleDateFormat("yyyyMMddHHmmssSSSS");
	public static SimpleDateFormat normalTimeFormat = new SimpleDateFormat("yyyyMMddHHmmss");
	public static SimpleDateFormat hourMinTimeFormat = new SimpleDateFormat("HH:mm:ss");
	...
	/**
	 * 
	 * 功能：解析数据库中的时间字符串 格式:yyyy年MM月
	 * 
	 * @param dateStr
	 * @return
	 */
	public static Date parseChinaDate(String dateStr) {
		Date date = null;
		try {
			date = dateChinaFormat.parse(dateStr);
		} catch (ParseException e) {
			e.printStackTrace();
		}
		return date;
	...
	/**
	 * 创建一个"yyyy年MM月"日期的格式化对象
	 * @return "yyyy年MM"日期的格式化对象
	 */
	private static SimpleDateFormat newLongChinaYMFormat() {
	 ...
	 */
	public static String getLongChinaYM() {
		return newLongChinaYMFormat().format(new Date());
	}
	...
	*/
	...
	public static int compareEndAndStart(String start, String end) throws Exception {
		...
		if (8 == start.length()) {
			...
			s = dateTimeFormat.parse(start).getTime();
		}
		long e = 0;
		if (8 == end.length()) {
			...
			e = dateTimeFormat.parse(end).getTime();
		}
		if (e > s) {
			return 1;
		} else if (e < s) {
	 ...
	 * 获取两个日期相差的月数
	 * 
	 * @param startTime (yyyy年MM月)
	 *            较小的日期
	 * @param endTime   (yyyy年MM月)
	 *            较大的日期
	 * @return 如果startTime < endTime 返回 月数差 否则返回0
	 * @throws ParseException
	 */
    public static int getMonthDiff(String startTime, String endTime) throws ParseException {
    	Calendar c1 = Calendar.getInstance();
        Calendar c2 = Calendar.getInstance();
        c1.setTime(parseChinaDate(startTime));
        c2.setTime(parseChinaDate(endTime));
        ...
        monthInterval %= 12;
        return Math.abs(yearInterval * 12 + monthInterval) + 1;
    }
	 ...
	 * @return
	 */
	public static Long calculateDiffDays(SimpleDateFormat simpleDateFormat, String startTime, String endTime) throws Exception {
		Date stateDate = simpleDateFormat.parse(startTime);
		Date endDate = simpleDateFormat.parse(endTime);
		return (endDate.getTime() - stateDate.getTime()) / (24*3600*1000);
	}
